...
async function getGroupAdmins(m) {
    if (!m.isGroup) return [];
    const groupMetadata = await m.client.groupMetadata(m.from).catch(() => null);
...
async function isAdmin(m) {
    ...
    const admins = await getGroupAdmins(m);
...
module.exports = {cutAudio,cutVideo,toAudio,toPTT,getGroupAdmins,isAdmin,isBotAdmin,getCompo,getDate,parsedJid,PREFIX,mode,extractUrlsFromString,getJson,isIgUrl,getUrl,isNumber,MediaUrls};
